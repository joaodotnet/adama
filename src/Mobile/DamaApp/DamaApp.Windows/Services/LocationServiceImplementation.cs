...
using DamaApp.Core.Services.Location;
using DamaApp.Windows.Services;
using System;
...
namespace DamaApp.Windows.Services
{
    public class LocationServiceImplementation : ILocationServiceImplementation
    {
        ...
        {
            var timeoutMilliseconds = timeout.HasValue ? (int)timeout.Value.TotalMilliseconds : DamaApp.Windows.Helpers.Timeout.Infinite;
            if (timeoutMilliseconds < 0 && timeoutMilliseconds != DamaApp.Windows.Helpers.Timeout.Infinite)
            ...
            var timer = new DamaApp.Windows.Helpers.Timeout(timeoutMilliseconds, pos.Cancel);
