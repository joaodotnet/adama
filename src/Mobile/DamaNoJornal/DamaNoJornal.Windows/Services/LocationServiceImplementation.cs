...
using DamaNoJornal.Windows.Services;
using System;
...
namespace DamaNoJornal.Windows.Services
{
    public class LocationServiceImplementation : ILocationServiceImplementation
    {
        ...
        {
            var timeoutMilliseconds = timeout.HasValue ? (int)timeout.Value.TotalMilliseconds : DamaNoJornal.Windows.Helpers.Timeout.Infinite;
            if (timeoutMilliseconds < 0 && timeoutMilliseconds != DamaNoJornal.Windows.Helpers.Timeout.Infinite)
            ...
            var timer = new DamaNoJornal.Windows.Helpers.Timeout(timeoutMilliseconds, pos.Cancel);
