...
{
    public static class SageOneAPIRequestSigner
    {
        /// <summary>
        ...
        public static string GenerateSignature(string httpMethod, Uri url, List<KeyValuePair<string, string>> requestBody, string signingSecret, string token, string nonce)
        {
            ...
            Debug.WriteLine($"httpMethod: {httpMethod}");
            ...
            Debug.WriteLine($"encodedParams: {encodedParams} from ({requestBody})");
            ...
            Debug.WriteLine($"encodedUri: {encodedUri} from ({url.GetLeftPart(UriPartial.Path).ToLower()})");

            // Escape the nonce
            string encodedNonce = Uri.EscapeDataString(nonce);
            Debug.WriteLine($"encodedNonce: {encodedNonce} from ({nonce})");

            string signingKey = Uri.EscapeDataString(signingSecret) + "&" + Uri.EscapeDataString(token);
            Debug.WriteLine($"signingKey: {signingKey} from ({signingKey},{token})");
            ...
            Debug.WriteLine($"baseString: {baseString}");
